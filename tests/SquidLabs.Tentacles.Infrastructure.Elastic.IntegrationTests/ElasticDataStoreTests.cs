...
public class ElasticSearchDataStoreTests
{
    private readonly IClientFactory<TestDataEntry, IElasticSearchOptions<TestDataEntry>, IElasticClient> _clientFactory;
    private readonly IDataStore<Guid, TestDataEntry> _dataStore;
    private readonly IOptionsMonitor<IElasticSearchOptions<TestDataEntry>> _elasticSearchOptions;

    public ElasticSearchDataStoreTests()
    {
        var mockOptionsMonitorForElastic = new Mock<IOptionsMonitor<TestElasticSearchptions>>();
        mockOptionsMonitorForElastic.Setup(monitor => monitor.CurrentValue).Returns(new TestElasticSearchptions
        {
            ConnectionDefinition = new[] { new Uri("http://localhost:9200") },
            IndexField = "Id"
        });

        _elasticSearchOptions = mockOptionsMonitorForElastic.Object;
        _clientFactory = new ElasticClientFactory<TestDataEntry>(_elasticSearchOptions);

        _dataStore = new ElasticSearchStore<Guid, TestDataEntry, ISearchRequest>(_clientFactory);
    }
